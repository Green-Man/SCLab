...
% dt_tab = [1/4 1/8];
eps_min = 1e-4; %border error in Newton's Method
nmax = 1000; %maximal number of steps for Newton's method
...
%Function needed for Newton's method (Adams Mouldon Method method)
...
for method=methods2,
    ...
    for i = 1:length(dt_tab)
        ...
        if strcmp(method.id,'euler'),
            Y = implicit_solver2(p0,dt,T,eps_min,nmax,p_fun_newton,p_fun_newton_prim,p_fun, method.id);
        else
            Y = implicit_solver2(p0,dt,T,eps_min,nmax,p_fun_newton2,p_fun_newton_prim2,p_fun, method.id);
        end
...
% Both linearisation methods ------------------

methods3 = struct('name', {'Adam Moulton 1st linearisation', 'Adam Moulton 2nd linearisation'},...
                                                                    'id', {'adams1', 'adams2'});
...
%Rows and Columns for Stability table
...
disp('Stable cases'); 
disp(stability_data);
disp('If the tables do not look pretty, please call Pavel. Have a wonderful day. :)')
disp('Just kidding, change font in command window to "Monospaced".');
